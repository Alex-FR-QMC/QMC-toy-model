...
if __name__ == "__main__":
    ...
    # ========================================================
    # Section 2 — Viability collapse (baseline)
    # Reproduces: figures/baseline.png
    # ========================================================
    ...
    print("=== Section 2 — Baseline summary ===")
    ...
    # ========================================================
    # Section 3.1 — Stochastic perturbations (noise sensitivity)
    # Reproduces: printed table in Section 3.1
    # 5 noise levels x 10 seeds
    # ========================================================

    seeds        = list(range(10))
    noise_levels = [0.01, 0.028, 0.05, 0.08, 0.12]

    print("\n=== Section 3.1 — Noise sensitivity ===")
    print(f"{'noise_std':>10}  {'mean_t_exit_V':>14}  {'mean_t_delta_collapse':>22}  {'mean_t_g_collapse':>18}  {'returned_to_V':>14}")
    for sigma in noise_levels:
        ...
        print(
            ...
            f"{summary['mean_t_delta_collapse']:>22.2f}  "
            f"{summary['mean_t_g_collapse']:>18.2f}  "
            f"{summary['fraction_returned_to_V']:>14.2f}"
        )

    # ========================================================
    # Section 3.2 — Point perturbation (shock)
    #
    # shock_weak  : amp=0.25 — absorbed without effect
    #               t_exit_V=9, returned_to_V=False
    #
    # shock_strong: amp=0.40 — transient reopening of V (4 steps)
    #               t_exit_V=9, retour t=80..83, returned_to_V=True
    #               Central result of Section 3.2: viability can be
    #               restored momentarily but not maintained without
    #               a regulatory architecture.
    #
    # Reproduces: figures/shock_weak.png, figures/shock_strong.png
    # ========================================================

    # Weak shock — absorbed
    shock_weak_cfg    = SimulationConfig(seed=42, shock_time=80, shock_node=3, shock_amplitude=0.25)
    shock_weak_result = run_simulation(shock_weak_cfg)
    plot_simulation(
        shock_weak_result,
        title="QMC Toy Model — Point Perturbation (t=80, amp=0.25, absorbed)",
        save_path="figures/shock_weak.png"
    )

    # Strong shock — transient reopening of V
    shock_strong_cfg    = SimulationConfig(seed=42, shock_time=80, shock_node=3, shock_amplitude=0.40)
    shock_strong_result = run_simulation(shock_strong_cfg)
    plot_simulation(
        shock_strong_result,
        title="QMC Toy Model — Point Perturbation (t=80, amp=0.40, transient reentry)",
        save_path="figures/shock_strong.png"
    )

    print("\n=== Section 3.2 — Point shock summary ===")
    for label, r in [("shock_weak  (amp=0.25)", shock_weak_result),
                     ("shock_strong (amp=0.40)", shock_strong_result)]:
        print(f"{label} | t_exit_V={r['t_exit_V']} | "
              f"t_delta_collapse={r['t_delta_collapse']} | "
              f"returned_to_V={r['returned_to_V']}")

    # ========================================================
    # Section 3.3 — Structural perturbations (rewiring)
    #
    # Late rewire  (t=60): graph reconfiguration after collapse —
    #              null effect, confirms irreversibility.
    #              Negative control.
    #
    # Early rewire (t=10): reconfiguration at the boundary of V —
    #              differentiated effects by mode:
    #              random_rewire : accelerates G collapse (t_G=32)
    #              densify       : 1-step transient reentry (artefact)
    #              sparsify      : slows collapse (t_G=38)
    #
    # Reproduces: figures/rewire_late.png, figures/rewire_early.png
    # ========================================================

    # Late rewire t=60 — negative control
    rewire_late_cfg    = SimulationConfig(seed=42, rewire_time=60, rewire_mode="random_rewire", rewire_fraction=0.20)
    rewire_late_result = run_simulation(rewire_late_cfg)
    plot_simulation(
        rewire_late_result,
        title="QMC Toy Model — Late Structural Perturbation (t=60, negative control)",
        save_path="figures/rewire_late.png"
    )

    # Early rewire t=10 — differentiated effects
    rewire_early_cfg    = SimulationConfig(seed=42, rewire_time=10, rewire_mode="random_rewire", rewire_fraction=0.20)
    rewire_early_result = run_simulation(rewire_early_cfg)
    plot_simulation(
    tau_clip_max: float = 0.91
